...
import com.raven.swing.PanelSlide;
                ...
                new Thread(new Runnable() {
                    ...
                    public void run() {
                        PublicEvent.getInstance().getEventMain().showLoading(true);
                         LOGGER.log(Level.INFO, "Attempting login for user: " + data.getUserName());
                        Service.getInstance().getClient().emit("login", data.toJsonObject(), new Ack() {
                            @Override
                            public void call(Object... os) {
                                 LOGGER.log(Level.INFO, "Login Ack callback received");
                                if (os.length > 0) {
                                     boolean action = (Boolean) os[0];
                                     LOGGER.log(Level.INFO, "Login action: " + action);
                                    if (action) {
                                        //  parse JSON Object into Model_User_Account
                                        try {
                                            if (os[1] != null) {
                                                 JSONObject jsonObject = new JSONObject(os[1].toString());
                                                Model_User_Account user = new Model_User_Account(jsonObject);
                                                 Service.getInstance().setUser(user);
                                                 PublicEvent.getInstance().getEventMain().showLoading(false);
                                                 PublicEvent.getInstance().getEventMain().initChat();
                                                 LOGGER.log(Level.INFO, "Login successful for user: " + user.getUserName());
                                            } else {
                                                //  handle the case where os[1] is null
                                                 LOGGER.log(Level.WARNING, "Login failed, user data is null from server");
                                                PublicEvent.getInstance().getEventMain().showLoading(false);
                                            }

                                        } catch(Exception e) {
                                             LOGGER.log(Level.SEVERE, "Error in processing login data ", e);
                                            PublicEvent.getInstance().getEventMain().showLoading(false);
                                        }

                                    } else {
                                        //  password wrong
                                         LOGGER.log(Level.WARNING, "Login failed, Wrong password");
                                        PublicEvent.getInstance().getEventMain().showLoading(false);
                                    }
                                } else {
                                     LOGGER.log(Level.WARNING, "Login failed, server response has no data");
                                    PublicEvent.getInstance().getEventMain().showLoading(false);
                                }
                            }
                        });
               ...
               Service.getInstance().getClient().emit("register", data.toJsonObject(), new Ack() {
                    ...
                    public void call(Object... os) {
                        if (os.length > 0) {
                            ...
                            if (ms.isAction()) {
                                try{
                                    JSONObject jsonObject = new JSONObject(os[2].toString());
                                     Model_User_Account user = new Model_User_Account(jsonObject);
                                      Service.getInstance().setUser(user);
                                } catch (Exception e) {
                                      LOGGER.log(Level.SEVERE, "Error in processing registration data", e);
                                }
                            }
